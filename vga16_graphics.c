...
unsigned char vga_data_array[TXCOUNT_LINE*5];
...
void initVGA() {
   ...
   pio_set_gpio_base(pio,16);
    ...
    for(int i=27;i<507;i++) {
      vga_address_table[i] = &vga_data_array[0] + TXCOUNT_LINE * 4; // Border is 193rd line
    }

    // TODO: Modify to 193 lines

    for(int i=0;i<192;i++) {  
//      vga_address_table[i*2+67+8] = &vga_data_array[0] + TXCOUNT_LINE * i;
//      vga_address_table[i*2+68+8] = &vga_data_array[0] + TXCOUNT_LINE * i;
      vga_address_table[i*2+67+8] = &vga_data_array[0] + TXCOUNT_LINE * (i%4);
      vga_address_table[i*2+68+8] = &vga_data_array[0] + TXCOUNT_LINE * (i%4);
